...
function [OUT,IN,OUTFILE] = func_init_arrays(C,grid,io)
    ...
    if grid.doubledepth                                                 % vertical layer depths (m)
        ...
        if length(grid.split)>1
            for n=2:length(grid.split)
                OUT.subZ(grid.mask_short==1,grid.split(n-1):grid.split(n)-1) = (2.0^(n-1))*grid.max_subZ;
            end
        end
        OUT.subZ(grid.mask_short==1,grid.split(end):grid.nl) = (2.0^length(grid.split))*grid.max_subZ;
    end
